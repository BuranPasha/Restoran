...
public class FPSInteraction : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    private GameObject heldObject = null;  // Şu anda tutulan nesne
    private Vector3 originalScale;  // Itemin orijinal scale'ini saklamak için bir değişken
    private bool isItemHeld = false;  // Item tutma durumu (local oyuncu için)
    private Rigidbody heldRigidbody;  // Tutulan nesnenin Rigidbody bileşeni
    private PhotonView heldObjectPhotonView;  // Nesnenin PhotonView bileşeni
    ...
    void TryPickUp()
    {
        ...
        if (Physics.Raycast(ray, out hit, interactionDistance))
        {
            if (hit.collider.CompareTag("Pickable"))
            {
                ...
                // Eğer nesne başka bir oyuncu tarafından alındıysa işlem yapma
                if (objectPhotonView.Owner != null && !objectPhotonView.IsMine)
                {
                    return; // Başka biri tutuyorsa itemi alamayız.
                }

                // Sahipliği al ve itemi tut
